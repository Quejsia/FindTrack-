...
const ONBOARD_STEPS = [
  {
    ...
    desc: "Report missing items, search found belongings, and coordinate returns from one focused workspace."
  },
  {
    icon: Package,
    label: "Step 2 of 4",
    title: "Report Lost or Found Items",
    desc: "Add a clear title, photo, location, and identifying details so other users can recognize the item quickly."
  },
  {
    icon: Sparkles,
    label: "Step 3 of 4",
    title: "Smart Match Suggestions",
    desc: "FindTrack compares reports and highlights possible matches so you can move from searching to verifying faster."
  },
  {
    icon: Bookmark,
    label: "Step 4 of 4",
    title: "Pin & Track Items",
    desc: "Bookmark listings you are watching and return to them from Pinned Items whenever you need an update."
  }
];

const NAV_ITEMS: { id: string; label: string; icon: LucideIcon; guestLocked?: boolean }[] = [
  { id: 'home', label: 'Home', icon: Home },
  { id: 'report', label: 'Report', icon: Package, guestLocked: true },
  { id: 'search', label: 'Search', icon: Search },
  { id: 'notifications', label: 'Alerts', icon: Bell, guestLocked: true },
  { id: 'profile', label: 'Profile', icon: UserCircle }
];

const GUIDE_TIPS = [
  { icon: Search, title: 'Retrace Recent Locations', body: 'Revisit the places you recently used, sat, paid, or packed up.' },
  { icon: MapPin, title: 'Check Nearby Areas', body: 'Inspect public spaces, offices, transport stops, shops, and common areas.' },
  { icon: Smartphone, title: 'Use Digital Tools', body: 'Post on FindTrack, relevant groups, and trusted community channels.' },
  { icon: RefreshCw, title: 'Act Quickly', body: 'Report and search as soon as possible while details are still fresh.' },
  { icon: Camera, title: 'Add Photos', body: 'Upload a clear image or reference photo to make identification faster.' },
  { icon: Bell, title: 'Stay Updated', body: 'Check messages and match alerts so you can respond while leads are active.' },
  { icon: FileText, title: 'Submit Detailed Reports', body: 'Include brand, color, marks, location, and timing for easier matching.' }
];

const PACKAGING_TIPS = [
  { icon: Box, title: 'Protect Fragile Items', body: 'Use padding or a secure container for delicate objects.' },
  { icon: Gift, title: 'Seal Securely', body: 'Keep items contained before storage or handoff.' },
  { icon: Building2, title: 'Classify Correctly', body: 'Hand keys and sensitive IDs straight to the Library security safe desk.' },
  { icon: RefreshCw, title: 'Update Status', body: "Mark items as claimed once they've been recovered." }
];
...
export default function App() {
  ...
  const cleanToastMessage = (msg: string) => msg.replace(/^[^\w\s]+/u, '').trim();
  ...
  const handleApproveClaim = async (claimId: string, itemId: string) => {
    try {
      ...
      triggerToast("Ownership claim approved. Access details unlocked.", "success");
    } catch (err) {
  ...
  const OnboardIcon = ONBOARD_STEPS[onboardStep].icon;
    ...
    <div className="relative min-h-screen bg-[#f0f4f8]">
      ...
      <div className="toast-container" id="toastContainer">
        {toasts.map(t => (
          <div key={t.id} className={`toast-msg ${t.type}`}>
            {t.type === 'success' ? <CheckCircle2 className="ui-icon toast-icon" /> : <AlertTriangle className="ui-icon toast-icon" />}
            <span>{cleanToastMessage(t.msg)}</span>
          </div>
        ))}
      </div>
        ...
        <div className="landing-page">
          ...
          <nav className="landing-nav">
            <div className="nav-logo">
              <div className="nav-logo-icon"><Search className="ui-icon" /></div>
              <span>FindTrack</span>
            </div>
          ...
          <div className="hero">
            ...
            <p>Report missing items, browse found belongings, and coordinate returns from one calm, organized workspace.</p>

            <div className="hero-actions">
              <button onClick={() => setCurrentView('signup')} className="btn-hero-primary"><FileText className="ui-icon" />Get Started Free</button>
              <button onClick={() => setCurrentView('login')} className="btn-hero-secondary"><LogIn className="ui-icon" />Sign In</button>
            </div>
          ...
          <div className="features">
            <div className="feat-card">
              <div className="feat-icon sky"><Package className="ui-icon" /></div>
            ...
            <div className="feat-card">
              <div className="feat-icon mint"><Search className="ui-icon" /></div>
            ...
            <div className="feat-card">
              <div className="feat-icon indigo"><BarChart3 className="ui-icon" /></div>
            ...
            <div className="feat-card">
              <div className="feat-icon amber"><Bell className="ui-icon" /></div>
          ...
          <footer className="landing-footer flex flex-col sm:flex-row justify-between items-center gap-4">
            <p>Built for practical lost-and-found recovery - FindTrack v2.0</p>
        ...
        <div className="landing-page flex items-center justify-center">
          <div className="auth-wrap">
            ...
            <div className="auth-logo">
              <div className="auth-logo-icon"><Search className="ui-icon" /></div>
              <h1>FindTrack</h1>
              ...
              <form onSubmit={handleLoginSubmit}>
                <div className="field">
                  ...
                  <div className="field-wrap">
                    <span className="field-icon"><Mail className="ui-icon" /></span>
                    <input 
                      type="email" 
                ...
                <div className="field">
                  ...
                  <div className="field-wrap">
                    <span className="field-icon"><Lock className="ui-icon" /></span>
                    <input 
                ...
                <button type="submit" className="btn-submit"><LogIn className="ui-icon" />Sign In</button>
              </form>
        ...
        <div className="landing-page flex items-center justify-center">
          <div className="auth-wrap">
            ...
            <div className="auth-card">
              ...
              <div className="card-sub">Set up your FindTrack profile</div>
              ...
              <form onSubmit={handleSignupSubmit}>
                <div className="fields-row">
                  <div className="field">
                    ...
                    <div className="field-wrap">
                      <span className="field-icon"><UserCircle className="ui-icon" /></span>
                      <input 
                        type="text" 
                        placeholder="Juan" 
                        value={signupFirst}
                ...
                <div className="field">
                  ...
                  <div className="field-wrap">
                    <span className="field-icon"><Smartphone className="ui-icon" /></span>
                    <input 
                      type="tel" 
                ...
                <button type="submit" className="btn-submit"><FileText className="ui-icon" />Create Account</button>
              </form>
      ...
      {currentView === 'verify-email' && (
        <div className="landing-page flex items-center justify-center">
          <div className="auth-wrap">
            ...
            <div className="auth-logo">
              <div className="auth-logo-icon"><Mail className="ui-icon" /></div>
              <h1>FindTrack</h1>
            ...
            <div className="auth-card" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    ...
                    try {
                      if (auth.currentUser) {
                        ...
                        if (auth.currentUser.emailVerified) {
                          ...
                          triggerToast("Email is not verified yet. Please check your inbox.", "error");
                        }
                      } else {
                ...
                >
                  <RefreshCw className="ui-icon" />Check Verification Status
                </button>
                ...
                >
                  <Send className="ui-icon" />Resend Verification Email
                </button>
              </div>
          ...
          <div style={{ maxWidth: '800px', width: '100%', background: 'rgba(30, 41, 59, 0.7)', border: '1px solid rgba(255, 255, 255, 0.1)', borderRadius: '24px', padding: '36px', backdropFilter: 'blur(20px)', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.3)' }} className="mx-auto">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px', borderBottom: '1px solid rgba(255, 255, 255, 0.1)', paddingBottom: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <Lock className="ui-icon page-icon" />
                <div>
      ...
      {currentView === 'terms' && (
        <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse at bottom, #1e293b 0%, #0f172a 100%)', color: '#f8fafc', padding: '40px 16px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <div style={{ maxWidth: '800px', width: '100%', background: 'rgba(30, 41, 59, 0.7)', border: '1px solid rgba(255, 255, 255, 0.1)', borderRadius: '24px', padding: '36px', backdropFilter: 'blur(20px)', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.3)' }} className="mx-auto">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px', borderBottom: '1px solid rgba(255, 255, 255, 0.1)', paddingBottom: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <ShieldCheck className="ui-icon page-icon" />
                <div>
        ...
        <div className="min-h-screen text-slate-800">
          ...
          <header className="topbar">
            <div className="topbar-inner">
              ...
              <div className="brand-wrap">
                <div className="logo"><Search className="ui-icon" /></div>
            ...
            <nav className="tabs">
              {NAV_ITEMS.map(({ id, label, icon: Icon, guestLocked }) => (
                <button
                  key={id}
                  onClick={() => {
                    if (id === 'home' || id === 'search') setCategoryKeywords(null);
                    if (guestLocked && profileName === 'Guest') setShowGuestModal(true);
          ...
          >
            <div className="drawer-header">
              ...
              <button id="closeDrawer" onClick={() => setSidebarOpen(false)}><X className="ui-icon" /></button>
            </div>
            <ul className="drawer-menu">
              ...
              <li 
                ...
                className="drawer-item"
              >
                <Bell className="ui-icon" /> Alerts
              </li>
              <li 
                onClick={() => { setActiveTab('profile'); setSidebarOpen(false); }} 
                className="drawer-item"
              >
                <UserCircle className="ui-icon" /> Profile
              </li>
              <hr />
              <li 
                onClick={() => { 
                ...
                className="drawer-item"
              >
                <Folder className="ui-icon" /> My Items
              </li>
              <li 
                onClick={() => { 
                  setSidebarOpen(false); 
                ...
                className="drawer-item"
              >
                <Bookmark className="ui-icon" /> Pinned Items
              </li>
              <li 
                onClick={() => { setActiveTab('categories'); setSidebarOpen(false); }} 
                className="drawer-item"
              >
                <Tags className="ui-icon" /> Categories
              </li>
              <li 
                onClick={() => { setActiveTab('analytics'); setSidebarOpen(false); }} 
                className="drawer-item"
              >
                <BarChart3 className="ui-icon" /> Analytics
              </li>
              <hr />
              <li 
                ...
                className="drawer-item"
              >
                <BookOpen className="ui-icon" /> Recovery Tips
              </li>
              <li 
                onClick={() => { setActiveTab('packaging'); setSidebarOpen(false); }} 
                className="drawer-item"
              >
                <Box className="ui-icon" /> Packaging Tips
              </li>
              <li 
                onClick={() => { setActiveTab('about'); setSidebarOpen(false); }} 
                className="drawer-item"
              >
                <Info className="ui-icon" /> About / Help
              </li>
              <li 
                onClick={() => { 
                  setSidebarOpen(false); 
                ...
                className="drawer-item"
              >
                <MessageCircle className="ui-icon" /> Feedback
              </li>
            </ul>
          </aside>
          ...
          <main>
                ...
                <div id="homeContent">
                  ...
                  <div className="welcome-card">
                    ...
                    <div className="stats-cards">
                      <div className="stat-card">
                        <div className="stat-icon"><MapPin className="ui-icon" /></div>
                        <div className="stat-label">Lost</div>
                        <div id="countLost" className="stat-value">{stats.lost}</div>
                      </div>
                      <div className="stat-card">
                        <div className="stat-icon"><Search className="ui-icon" /></div>
                        <div className="stat-label">Found</div>
                        <div id="countFound" className="stat-value">{stats.found}</div>
                      </div>
                      <div className="stat-card">
                        <div className="stat-icon"><CheckCircle2 className="ui-icon" /></div>
                  ...
                  <div className="recent-section">
                    <div className="recent-header">
                      <h3><ClipboardList className="ui-icon" /> Recent Reports Feed</h3>
                    </div>
                        ...
                        <div key={r.id} onClick={() => { setSelectedItemId(r.id); setActiveTab('itemDetail'); }} className="recent-item">
                          <div className="recent-thumb">
                            {r.image || r.imageUrl ? (
                              ...
                              r.type === 'lost' ? <MapPin className="ui-icon" /> : <Search className="ui-icon" />
                            )}
                          </div>
                  ...
                  <div className="tip-banner"><Info className="ui-icon" /> Tip: report lost items within 24 hours for the best chance of recovery.</div>
                  ...
                  <footer style={{ marginTop: '32px', paddingTop: '20px', borderTop: '1px solid rgba(0, 0, 0, 0.08)', display: 'flex', flexDirection: 'column', gap: '12px', alignItems: 'center', justifyContent: 'space-between', fontSize: '12px', color: '#64748b' }} className="sm:flex-row">
                    <p>(c) {new Date().getFullYear()} FindTrack - Lost &amp; Found System</p>
            ...
            <section id="report" className={`panel ${activeTab === 'report' ? 'active' : ''}`}>
              <div className="section-title"><Package className="ui-icon" /> Report Lost / Found Item</div>
              <p className="section-subtitle">Add clear details so the right person can identify the item quickly.</p>
              <div className="report-form-wrap">
                <form onSubmit={handleReportSubmit} id="reportForm">
                  ...
                  <div className="form-group bg-slate-50 border border-slate-205 rounded-xl p-4 my-2" style={{ backgroundColor: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '16px', margin: '8px 0' }}>
                    <label htmlFor="r_securityQuestion" className="text-slate-800 font-bold" style={{ fontWeight: 'bold', color: '#1e293b' }}><KeyRound className="ui-icon inline-icon" /> Owner verification question (optional)</label>
                  ...
                  <button className="primary-btn" type="submit" disabled={isUploading}>
                    {isUploading ? 'Uploading and submitting...' : <><Send className="ui-icon" /> Submit Report</>}
                  </button>
                </form>
              </div>
            </section>
            ...
            <section id="search" className={`panel ${activeTab === 'search' ? 'active' : ''}`}>
              <div className="section-title"><Search className="ui-icon" /> Search Database</div>
              
              <div className="search-container">
                <div className="search-bar">
                  <div className="search-input-wrapper">
                    <span className="search-icon"><Search className="ui-icon" /></span>
                    <input 
                      id="s_query" 
                  ...
                  >
                    <SlidersHorizontal className="ui-icon" /> Filters
                  </button>
                </div>
              ...
              {smartMatches.length > 0 && (
                <div id="matchBanner" className="match-banner show">
                  <div className="match-banner-title"><Sparkles className="ui-icon" /> Smart suggestions - possible matches for your query</div>
                    ...
                    {smartMatches.map(({ report, score }) => {
                      ...
                      return (
                        <div key={report.id} onClick={() => { setSelectedItemId(report.id); setActiveTab('itemDetail'); }} className="match-chip">
                          ...
                          <div className="match-chip-meta"><MapPin className="ui-icon inline-icon" /> {report.location || "Unknown"}</div>
                          <div className="match-score"><Target className="ui-icon inline-icon" /> {pct}% match</div>
                        </div>
                      );
                    })}
                  </div>
                      ...
                      <div className="relative">
                        <div className="card-media">
                          {r.image || r.imageUrl ? (
                            ...
                            <div className="empty-media-icon"><Camera className="ui-icon" /></div>
                          )}
                        </div>
                        <button 
                        ...
                        >
                          {pinned ? <Bookmark className="ui-icon" /> : <MapPin className="ui-icon" />}
                        </button>
                      </div>
            ...
            <section id="notifications" className={`panel ${activeTab === 'notifications' ? 'active' : ''}`}>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2 space-y-6">
                  ...
                  <div className="p-5 bg-slate-50/50 border border-slate-200/60 rounded-3xl space-y-4" id="finder-claims-review-panel">
                    <div className="flex flex-col gap-1.5 items-start sm:flex-row sm:items-center sm:justify-between">
                      <span className="text-sm font-bold text-slate-800 font-sans flex items-center gap-1.5 flex-wrap"><KeyRound className="ui-icon" /> Incoming Ownership Claims ({incomingClaims.filter(c => c.status === 'pending').length} pending)</span>
                              ...
                              {isPending ? (
                                ...
                                <p className="text-right text-[10px] text-slate-400 mt-2.5 font-sans font-medium">
                                  {claim.status === 'approved' 
                                    ? 'Approved: private contact details are now shared.' 
                                    : 'Declined claim.'}
                                </p>
                              )}
                            </div>
                  ...
                  <div className="section-title flex items-center justify-between">
                    <span className="flex items-center gap-1.5"><MessageCircle className="ui-icon" /> Active Chats Inbox</span>
                ...
                <div className="md:col-span-1 space-y-4">
                  <div className="section-title"><Bell className="ui-icon" /> Platform Alerts</div>
                  <div id="alertsList" className="space-y-3">
                    <div className="alert-item m-0">
                      <strong><CheckCircle2 className="ui-icon inline-icon" /> Welcome to Lost &amp; Found</strong>
                      <p>You'll receive secure notifications and match recommendations here.</p>
                    </div>
                    <div className="alert-item m-0">
                      <strong><Info className="ui-icon inline-icon" /> Good to know</strong>
            ...
            <section id="profile" className={`panel ${activeTab === 'profile' ? 'active' : ''}`}>
              <div className="section-title"><UserCircle className="ui-icon" /> My Profile</div>
              <div className="profile-container">
                <div className="profile-photo">
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '12px' }}>
                    <button onClick={handleRandomAvatar} className="secondary-btn" style={{ width: '100%', justifyContent: 'center' }}>
                      <Dice5 className="ui-icon" /> Random Avatar
                    </button>
                  </div>
                </div>
                ...
                <div className="profile-fields">
                  ...
                  <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center', marginTop: '4px' }}>
                    <button onClick={handleSaveProfile} className="primary-btn"><Save className="ui-icon" /> Save Profile</button>
                    <button 
                      onClick={handleLogoutAction} 
                      style={{ fontSize: '13px', color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}
                    >
                      <LogOut className="ui-icon inline-icon" /> Logout
                    </button>
                  </div>
                </div>
              </div>
            ...
            <section id="myitems" className={`panel ${activeTab === 'myitems' ? 'active' : ''}`}>
              <div className="section-title"><Folder className="ui-icon" /> My Items</div>
              <p className="section-subtitle">All items you've reported. Tap a listing to view details.</p>
                ...
                {items.filter(item => item.userId === auth.currentUser?.uid).map(r => {
                  ...
                  return (
                    <div key={r.id} className="card-item">
                      ...
                      <button className="delete-btn" onClick={() => deleteItem(r.id)}><Trash2 className="ui-icon inline-icon" /> Delete</button>
                    </div>
                  );
                })}
              </div>
            ...
            <section id="pinned" className={`panel ${activeTab === 'pinned' ? 'active' : ''}`}>
              <div className="section-title"><Bookmark className="ui-icon" /> Pinned Items</div>
            ...
            <section id="categories" className={`panel ${activeTab === 'categories' ? 'active' : ''}`}>
              <div className="section-title"><Tags className="ui-icon" /> Browse by Category</div>
              ...
              <div className="cards-grid">
                <div onClick={() => { setCategoryKeywords(["bag", "backpack", "purse", "wallet", "luggage", "suitcase", "handbag"]); setActiveTab('search'); }} className="card-item clickable">
                  <div className="card-media category-icon"><Backpack className="ui-icon" /></div>
                ...
                <div onClick={() => { setCategoryKeywords(["phone", "laptop", "tablet", "charger", "headphone", "earphone", "computer", "iphone", "samsung", "ipad", "macbook"]); setActiveTab('search'); }} className="card-item clickable">
                  <div className="card-media category-icon"><Smartphone className="ui-icon" /></div>
                ...
                <div onClick={() => { setCategoryKeywords(["book", "notebook", "textbook", "pen", "pencil", "id", "card", "stationery", "notes"]); setActiveTab('search'); }} className="card-item clickable">
                  <div className="card-media category-icon"><LibraryBig className="ui-icon" /></div>
                ...
                <div onClick={() => { setCategoryKeywords(["jacket", "shirt", "pants", "uniform", "glasses", "watch", "coat", "shoes", "hat", "scarf"]); setActiveTab('search'); }} className="card-item clickable">
                  <div className="card-media category-icon"><Shirt className="ui-icon" /></div>
            ...
            <section id="analytics" className={`panel ${activeTab === 'analytics' ? 'active' : ''}`}>
              <div className="section-title"><BarChart3 className="ui-icon" /> Analytics Dashboard</div>
            ...
            <section id="tips" className={`panel ${activeTab === 'tips' ? 'active' : ''}`}>
              <div className="section-title"><BookOpen className="ui-icon" /> Lost Item Recovery Guide</div>
              <p className="section-subtitle">Practical steps that make reports easier to match and verify.</p>
              <div className="tips-grid">
                {GUIDE_TIPS.map(({ icon: Icon, title, body }) => (
                  <div key={title} className="tip-card">
                    <span className="tip-card-icon"><Icon className="ui-icon" /></span>
                    <strong>{title}</strong>
                    <p>{body}</p>
                  </div>
                ))}
            ...
            <section id="packaging" className={`panel ${activeTab === 'packaging' ? 'active' : ''}`}>
              <div className="section-title"><Box className="ui-icon" /> Packaging &amp; Handling Tips</div>
              <p className="section-subtitle">Best practices for securing found items</p>
              <div className="tips-grid">
                {PACKAGING_TIPS.map(({ icon: Icon, title, body }) => (
                  <div key={title} className="tip-card">
                    <span className="tip-card-icon"><Icon className="ui-icon" /></span>
                    <strong>{title}</strong>
                    <p>{body}</p>
            ...
            <section id="about" className={`panel ${activeTab === 'about' ? 'active' : ''}`}>
              <div className="section-title"><Info className="ui-icon" /> About FindTrack</div>
              <div className="report-form-wrap" style={{ maxWidth: '600px' }}>
                <p style={{ marginBottom: '16px', lineHeight: 1.7 }}>
                  <strong>FindTrack</strong> keeps lost-and-found reports organized, searchable, and easier to verify.
                </p>
                <div className="section-title" style={{ fontSize: '16px', marginTop: '8px' }}><BookOpen className="ui-icon" /> How to Use</div>
                <div className="tips-grid" style={{ gridTemplateColumns: '1fr' }}>
                  <div className="tip-card compact"><Package className="ui-icon inline-icon" /> <strong>Report</strong> Submit details about lost or found items with photos and location.</div>
                  <div className="tip-card compact"><Search className="ui-icon inline-icon" /> <strong>Search</strong> Browse reports with filters and category shortcuts.</div>
                  <div className="tip-card compact"><CheckCircle2 className="ui-icon inline-icon" /> <strong>Claim</strong> Mark items as recovered, or manage your own reports.</div>
                  <div className="tip-card compact"><Bookmark className="ui-icon inline-icon" /> <strong>Pin</strong> Bookmark listings you want quick access to.</div>
                </div>
                <div className="tip-banner" style={{ marginTop: '16px' }}><Info className="ui-icon" /> More specific reports are easier to match and verify.</div>
              </div>
            </section>
          ...
          <nav className="bottom-nav" id="bottomNav">
            {NAV_ITEMS.filter(({ id }) => ['home', 'search', 'notifications', 'profile'].includes(id)).map(({ id, label, icon: Icon, guestLocked }) => (
              <button
                key={id}
                onClick={() => {
                  if (id === 'home' || id === 'search') setCategoryKeywords(null);
              ...
              >
                <span className="bnav-icon"><Icon className="ui-icon" /></span>{label}
              </button>
            ))}
          </nav>
      ...
      {zoomImg && (
        <div className="zoom-overlay" onClick={() => setZoomImg(null)}>
          <button className="zoom-close" onClick={() => setZoomImg(null)}><X className="ui-icon" /></button>
      ...
      {showGuestModal && (
        <div id="guestModal" className="modal" onClick={(e) => { if ((e.target as HTMLElement).id === 'guestModal') setShowGuestModal(false); }}>
          <div className="modal-content">
            <div className="modal-icon"><Lock className="ui-icon" /></div>
            <h2>Login Required</h2>
            <p>Please log in or create an account to report, claim, and message safely.</p>
            <div className="modal-buttons">
              <button onClick={() => { setShowGuestModal(false); setCurrentView('login'); }} className="modal-btn primary"><LogIn className="ui-icon" /> Login</button>
              <button onClick={() => { setShowGuestModal(false); setCurrentView('signup'); }} className="modal-btn secondary"><FileText className="ui-icon" /> Sign Up</button>
            </div>
