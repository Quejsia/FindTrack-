...
export default function ItemDetail({
  ...
  if (claimView) {
    return (
      <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm" style={{ height: '100dvh' }} id="dedicated-claim-page">
        <style>{`
          @keyframes shake {
        ...
        {/* Centered modal container with mobile-safe sizing */}
        <div className="relative w-full max-w-2xl flex flex-col bg-white rounded-md shadow-2xl border border-slate-700" style={{ maxHeight: '85dvh' }}>

          {/* HEADER (shrink-0) */}
          <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-teal-800 to-teal-600 text-white shrink-0 rounded-t-md shadow-md z-10 border-b border-teal-900/30">
            <div className="flex flex-col">
              <span className="text-2xl font-black text-white leading-tight tracking-tight">Log Ownership Claim</span>
              <span className="text-teal-100 text-[10px] font-mono font-bold tracking-widest uppercase text-opacity-90 mt-1">Prove-It Verification Layer</span>
            </div>
            <button
              type="button"
              onClick={() => setClaimView(false)}
              className="text-white hover:text-teal-50 bg-teal-900/40 hover:bg-teal-900/70 px-3 py-2 rounded border border-teal-500/30 text-xs font-bold uppercase tracking-wider transition cursor-pointer shadow-sm active:scale-95"
          ...
          {/* MIDDLE: scrollable content (flex-1 overflow-y-auto) */}
          ...
          {/* FOOTER (shrink-0): persistent CTA area */}
          <div className="px-6 py-4 bg-white border-t border-slate-100 flex items-center gap-3 shrink-0">
            <button
              type="button"
              onClick={() => setClaimView(false)}
              className="px-4 py-2 border-2 border-[#008080] rounded-md text-[#008080] bg-white text-sm font-bold transition shadow-sm hover:bg-slate-50 active:scale-95"
            >
              Cancel
            </button>
            ...
            <button
              ...
              className="ml-auto px-5 py-2 border-2 border-[#008080] rounded-md bg-[#008080] hover:bg-teal-700 disabled:bg-teal-400 disabled:border-teal-400 text-white font-bold text-sm flex items-center justify-center gap-2 shadow-md transition active:scale-95"
            >
              {submittingClaim ? (
                <>
